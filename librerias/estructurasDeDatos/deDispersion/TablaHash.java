...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        ...
        for (int i = 0; i < elArrayAntiguo.length; i++) {
            ListaConPI<EntradaHash<C, V>> l = elArrayAntiguo[i];
            for (l.inicio(); !l.esFin(); l.siguiente()) {
                //OJO insertar cada entrada en la cubeta
                //l EN LA NUEVA TABLA, i.e. en this
                EntradaHash<C, V> e = l.recuperar();
                this.insertar(e.clave, e.valor);
                
                //Alternativa mas eficiente (essiste,pero no hace falta)
    ...
    public final double desviacionTipica() {
        /* COMPLETAR */
        double res = 0, resta = 0, factCarga = factorCarga();
        for(int i = 0; i < elArray.length; i++) {
            resta = elArray[i].talla() - factCarga;
            res += resta * resta;
        }
        res = res / elArray.length;
